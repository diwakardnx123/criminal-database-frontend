...
export default function UploadPage() {
  ...
  // Check if at least one file has been uploaded to enable the execute button
  const hasFiles = Object.values(files).some((file) => file !== null);

  const handleExecute = () => {
    setIsProcessing(true);
    // Simulate backend processing time before navigating
    ...
    <div className="pointer-events-auto flex flex-col h-full w-full max-w-5xl mx-auto mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="col-span-2 bg-[#051329]/50 border border-cyan-800/40 p-6 rounded-lg backdrop-blur-md flex flex-col">
          ...
          {/* Render the 3 separate upload buttons */}
          <div className="flex-1 flex flex-col gap-4 justify-center">
            {ingestionModules.map((mod) => (
              ...
              >
                ...
                {/* Icon Container */}
                <div className={`w-12 h-12 flex items-center justify-center rounded-lg mr-4 transition-colors
                  ${files[mod.id] ? 'bg-green-500/20 text-green-400' : 'bg-cyan-900/40 text-cyan-500 group-hover:text-cyan-300'}`}>
                  {files[mod.id] ? (
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-xs mt-1 truncate max-w-[200px] sm:max-w-[300px] ${files[mod.id] ? 'text-green-500/70' : 'text-cyan-700'}`}>
          ...
          >
            {isProcessing ? 'Correlating Data...' : 'Initialize Analysis'}
          </button>
        </div>
      </div>
    </div>
